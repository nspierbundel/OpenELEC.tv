...
K_db='http://xty.me/xunitytalk/addons/plugin.video.MikeysKaraoke/Karaoke.db'
updatetxt='http://xty.me/xunitytalk/addons/plugin.video.MikeysKaraoke/update.txt'
...
def SEARCH(url):
        ...
        for links in link:
            if not '__video_id__' or not '__title__' in links:
                match=re.compile('href=".+?=(.+?)".+?dir="ltr">(.+?)</a>').findall(links)
                for items in match:
                    url= match[0][0]
                    name=match[0][1]
