...
def doRegex(murl):
    ...
    doRegexsONE = str(doRegexs)
    print "DO REGEX IS " + doRegexsONE
    for k in doRegexs:
        if k in murl:
            print "YES THERE IS K = " +k
            regex=re.compile('<name>'+k+'</name><expres>(.+?)</expres><page>(.+?)</page><referer>(.+?)</referer></regex>',re.DOTALL).search(murl)
            referer=regex.group(3)
            page=regex.group(2)
            express=regex.group(1)
            print "Referer is = " +referer
            print "Page is = "+page
            print "Express is = "+express
            if referer=='':
                print "REFERER IS NOTHING"
...
def updateSearchFile(searchQuery,searchType,defaultValue = '###',searchMsg = ''):
    ...
    if searchType == "TV":
        ...
        searchHistoryFile = "PhoenixSearchHistory"
...
def batchOPENURL(urls, mobile = False, merge = True):
    ...
    for url in urls: 
        q = queue.Queue()
        threading.Thread(target=OPEN_S_URL, args=(url,mobile,q)).start()
        results.append(q)
    if merge: content = ''
    else: content = []
    for n in range(max):
...
def OPEN_S_URL(url, mobile = False, q = False, verbose = True, timeout = 10, cookie = None, data = None, cookiejar = False, log = True, headers = [], type = '',ua = False):
    import urllib2 
