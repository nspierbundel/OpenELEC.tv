...
schedule_path  = os.path.join(xbmc.translatePath('special://profile/addon_data'), 'plugin.video.sportingsstreams.schedule.txt')
channels_path  = os.path.join(xbmc.translatePath('special://profile/addon_data'), 'plugin.video.sportingstreams.channels.txt')
upcoming_path  = os.path.join(xbmc.translatePath('special://profile/addon_data'), 'plugin.video.sportingstreams.upcoming.txt')
settings    = xbmcaddon.Addon("plugin.video.sportingstreams");
...
def login():
	...
	url = 'http://sportingstreams.net/api/?request=login'
	url2 = 'http://sportingstreams.net/watch/livehds'
	url3 = 'http://sportingstreams.net/watch/upcoming-matches'
	req = mechanize.Browser()
	req.addheaders = [('User-agent','Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:19.0) Gecko/20100101 Firefox/19.0')]
	req.addheaders = [('Referer','http://footyreport.net/')]
...
def get_rtmp_url(path):
	streamer  = 'rtmp://vdn.sportingstreams.net:443/live'
	playPath  = path
	appUrl    = 'live' 
	rtmp = ''.join([streamer,' playpath=', playPath,'?s=MBXrf',' app=', appUrl, ' live=true'])
	return rtmp
