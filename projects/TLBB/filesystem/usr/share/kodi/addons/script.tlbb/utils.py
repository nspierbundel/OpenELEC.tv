...
def log(text):
    #xbmc.log("[TLBB] : %s" % str(text), xbmc.LOGDEBUG)
    xbmc.log("[TLBB] : %s" % str(text))
...
def rebootCommand(silent):
    ...
    if silent=='2':
        perform=getString(103)
        cmd='reboot recovery'
    else:
        perform=getString(104)
        cmd='factoryreset'
...
def getAmlogicCpuType():
    cpu = 'unknown'
    with open('/proc/cpuinfo', 'r') as fin:
        info = fin.read()

        if 'MESON-M3' in info or 'MESON3' in info:
            cpu = 'm3'
        elif 'Meson6' in info:
            cpu = 'm6'
        elif 'Meson8' in info:
