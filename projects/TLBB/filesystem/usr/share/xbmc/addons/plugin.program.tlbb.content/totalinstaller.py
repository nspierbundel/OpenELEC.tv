...
FANART              = utils.FANART
XML_EXPRESSION_ITEM = '<item><title>(.+?)</title><icon>(.+?)</icon><repoLink>(.+?)</repoLink><pluginLink>(.+?)</pluginLink><cmd>(.+?)</cmd><thumbnail>(.+?)</thumbnail><rating>(.+?)</rating><type>(.+?)</type><description>(.+?)/|^$</description></item>'
...
def add2HELPItem(name, url, cmd, mode, iconimage, fanart, description, filetype, rating, isFolder=True):
        u  = "plugin://plugin.program.tlbb.content/"
        u += "?url="         + urllib.quote_plus(url)
        u += "&name="        + urllib.quote_plus(name)
        u += "&filetype="    + urllib.quote_plus(filetype)
...
def ADDONINSTALL(name, url, cmd, filetype, repourl, fav_path):
    try:
        ...
        #On install
            ...
            for title,icon,repolink,pluginlink,cmd,thumbnail,rating,type,description in match:
                if title == name:
...
def ADDONLIST(url,mode):
    ...
    if mode == "official":
        ...
        for title,icon,repolink,pluginlink,cmd,thumbnail,rating,type,description in matchItem:
            if type == "Official":
                add2HELPItem(title, repolink, cmd,  552, icon, thumbnail, description[0:-1], 'addon', rating,)                     
